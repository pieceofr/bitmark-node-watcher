...
func (w *NodeWatcher) pullImage() (updated bool, err error) {
	...
	//log.Println(string(response), "contains download", strings.Contains(string(response), newImageDownloadIndicator))
	return
}
...
func (w *NodeWatcher) getContainersWithImage() ([]types.Container, error) {
	...
	if err != nil {
		log.Println("Error in getCurContainers:", err)
		return nil, err
	}
	...
	for _, container := range containers {
		...
		if err != nil {
			log.Println("Error: getTartgetContainer: ContainerInspect")
			return targetContainers, err
		}

		if container.Image == w.ImageName {
			log.Println("Target Container Found")
			targetContainers = append(targetContainers, container)
		}
	}
	return targetContainers, nil
}
func (w *NodeWatcher) getOldContainers() ([]types.Container, error) {
	//get all containers
	containers, err := w.DockerClient.ContainerList(w.BackgroundContex, types.ContainerListOptions{All: true})
	if err != nil {
		log.Println("Error in getCurContainers:", err)
...
func (w *NodeWatcher) stopContainers(containers []types.Container, stopTimeout time.Duration) error {
	for _, container := range containers {
		//stop container
		log.Println("stop container state:", container.State)
...
func (w *NodeWatcher) startContainers(containerID string) error {
...
func (w *NodeWatcher) forceRemoveContainers(containerID string) error {
...
func (w *NodeWatcher) removeContainers(containerID string) error {
...
func (w *NodeWatcher) renameContainers(containers []types.Container, postfix string) error {
	for _, c := range containers {
		newName := c.Names[0] + postfix
		err := w.DockerClient.ContainerRename(w.BackgroundContex, c.ID, newName)
		if err != nil {
			log.Println("renameContainers error:", err)
			return err
		}
		log.Println("Container:", c.ID, " is rename to ", newName)
	}
	return nil
}

func (w *NodeWatcher) getContainer(c []types.Container) *types.Container {
	compareName := "/" + w.ContainerName
	for _, container := range c {
		for _, n := range container.Names {
			log.Println("container name:", n)
			if n == compareName {
				return &container
			}
		}
